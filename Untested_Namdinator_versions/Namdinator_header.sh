...
############################################################################
############# Extracting CRYST1. ORIGX and SCALE recoords ##################
############################################################################

grep -E '^CRYST1|^ORIGX[1-3]|^SCALE[1-3]' $PDBIN > header.txt
...
if [ "$LIGANDS" = "1" ]; then
    ...
    grep "HETATM\|^TER\|END\|^ATOM" $PDB1-tmp.pdb > ${PDB1}_altered.pdb
    
 else

    grep "^TER\|END\|^ATOM" $PDB1-tmp.pdb > ${PDB1}_altered.pdb
...
cat header.txt last_frame_nucleo.pdb > last_frame.pdb

rm last_frame_nucleo.pdb

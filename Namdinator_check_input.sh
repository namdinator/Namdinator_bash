...
if [ -s duplicate_atoms.plt ]; then
    ...
    echo -n "Identified duplicated atoms within "$PDBIN" which is not allowed.
Please fix the below listed duplicats before trying again. 
The number in front of the atom labels, indicate how many times the specific atom exist within the file
"
...
if [ -s mislabeled_atoms.plt ]; then
    ...
    echo "Identified some mislabeled atoms"
...
if [[ -s mislabeled_atoms.plt || -s duplicate_atoms.plt ]]; then

    echo " Problems with the input PDB file, check the info above for details"
...
if grep -A5 'FATAL ERROR: Bad global bond count!' NAMD2_step1.log; then

    echo -n '

NAMD2 have unfortunately stopped prematurely due to a fatal error involving global bond counts. These types of errors generally indicate problems with the input PDB file. It could be a question of nonbonded atoms which is placed in different non-neighboring patches during autoPSF. One way to check this is to load the psf and pdb file generated by autoPSF in VMD and look for very long bonds to hydrogens, and then fix it manually.

Another possibility is that some atoms are specified more than once in the PDB file. e.g. one atom/residue ends up in two chains or one atom/residue is specified twice in one chain etc. This is very difficult to spot manually, but one way to identify it could be to run phenix.molprobity on the input PDB file and look for duplicate atoms in the pre-test of the PDB file it performs.

It could also be due to severe clashes with in the input PDB file, as in the backbone is threaded through an aromatic side chain or that backbone chains are interweaved with no possible way to be seperated except via bond breakage etc. 
'
    exit 1
fi
...
score_jd2.mpi.linuxgccrelease -in:file:s ${PDB}.pdb -ignore_unrecognized_res -edensity::mapfile ${MAP}.mrc -edensity::mapreso ${RES} -edensity:sliding_window_wt 2.0 -edensity:sliding_window 3 -edensity::cryoem_scatterers -crystal_refine -out:file:scorefile ${PDB}.sc > ${PDB}_rosetta.log

score_jd2.mpi.linuxgccrelease -in:file:s last_frame.pdb -ignore_unrecognized_res -edensity::mapfile ${MAP}.mrc -edensity::mapreso ${RES} -edensity:sliding_window_wt 2.0 -edensity:sliding_window 3 -edensity::cryoem_scatterers -crystal_refine -out:file:scorefile lf.sc > lf_rosetta.log
...
score_jd2.mpi.linuxgccrelease -in:file:s ${PDB}.pdb -ignore_unrecognized_res -edensity::mapfile ${MAP}.mrc -edensity::mapreso ${RES} -edensity:sliding_window_wt 4.0 -edensity:sliding_window 3 -edensity::cryoem_scatterers -crystal_refine -out:file:scorefile ${PDB}.sc > ${PDB}_rosetta.log

score_jd2.mpi.linuxgccrelease -in:file:s last_frame.pdb -ignore_unrecognized_res -edensity::mapfile ${MAP}.mrc -edensity::mapreso ${RES} -edensity:sliding_window_wt 4.0 -edensity:sliding_window 3 -edensity::cryoem_scatterers -crystal_refine -out:file:scorefile lf.sc > lf_rosetta.log
...
score_jd2.mpi.linuxgccrelease -in:file:s ${PDB}.pdb -ignore_unrecognized_res -edensity::mapfile ${MAP}.mrc -edensity::mapreso ${RES} -edensity:fastdens_wt 20.0 -edensity::cryoem_scatterers -crystal_refine -out:file:scorefile ${PDB}.sc > ${PDB}_rosetta.log

score_jd2.mpi.linuxgccrelease -in:file:s last_frame.pdb -ignore_unrecognized_res -edensity::mapfile ${MAP}.mrc -edensity::mapreso ${RES} -edensity:fastdens_wt 20.0 -edensity::cryoem_scatterers -crystal_refine -out:file:scorefile lf.sc > lf_rosetta.log

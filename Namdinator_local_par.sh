...
phenix.emringer last_frame.pdb $MAPIN nproc=$PROCS > last_EMR.log
phenix.emringer $PDB.pdb $MAPIN nproc=$PROCS > ${PDB}_EMR.log
...
sh rosetta_resi.sh & PID[rosetta]=$!
...
while:
do
    if [[ 'lsof -p $PID[rosetta]' ]]
    then
	   echo "Waiting for Rosetta to complete"
